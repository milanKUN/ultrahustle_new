...
export default function LoginPage() {
  // View states: 'login' | 'forgot' | 'verify' | 'resetSuccess'
  ...
  const renderLoginView = () => (
    <>
      ...
      <div className="flex bg-white/10 rounded-2xl p-1 mb-6 relative border border-white/80">
        ...
        <Link to="/signup" className="flex-1 py-3 text-center rounded-xl text-gray-700 font-semibold text-sm hover:bg-white/10 transition-all uppercase tracking-wide">
          Sign up
        </Link>
      </div>
      ...
      <form className="space-y-4">
        <div className="space-y-1.5">
          <label className="text-gray-700 font-medium text-sm ml-1">Email Address or phone number</label>
          <input 
            type="text" 
...
>
  Login
      ...
      <div className="relative my-5 text-center">
        ...
        <span className="relative px-4 text-xs font-bold text-gray-600 uppercase tracking-widest bg-transparent">OR</span>
      </div>
      ...
      <p className="text-center mt-6 text-gray-800 font-medium text-sm">
        Don't have an account? <Link to="/signup" className="font-bold border-b-2 border-gray-900 pb-0.5 hover:opacity-70 transition-opacity">Sign up</Link>
      </p>
    </>
  );
  ...
  const renderForgotView = () => (
    <>
      <div className="text-center mb-8 relative z-10">
        ...
        <p className="text-gray-600 text-sm">No worries, we'll send you the reset instructions</p>
      </div>

      <form onSubmit={handleForgotSubmit} className="space-y-5 relative z-10">
        <div className="space-y-1.5">
          <label className="text-gray-700 font-medium text-sm ml-1">Email Id or Phone Number</label>
          <input 
            type="text" 
            placeholder="dev@gmail.com"
            value={email}
  ...
  const renderVerifyView = () => (
    <>
      <div className="text-center mb-8 relative z-10">
        ...
        <p className="text-gray-600 text-sm">Enter the 6 digit code below to reset your password</p>
      </div>
  ...
  const renderResetSuccessView = () => (
    <>
      ...
      <div className="text-center mb-8 relative z-10">
        ...
        <p className="text-gray-600 text-sm">
          Your password has been successful reset.<br />Click below to log in.
        </p>
      </div>
  ...
  return (
    ...
    >
      <div className={`w-full ${currentView === 'login' ? 'max-w-[600px]' : 'max-w-[550px]'} backdrop-blur-xl bg-white/5 border border-white/20 shadow-2xl rounded-[40px] p-6 md:p-8 relative overflow-hidden mx-4 md:mx-0 z-10 transition-all hover:bg-white/10`}>
