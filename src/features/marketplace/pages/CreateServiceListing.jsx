...
export default function CreateServiceListing({ theme, setTheme }) {
  /* ================== CONSTANTS ================== */
  const categories = useMemo(() => ["Design", "Development", "Marketing", "Writing"], []);
  ...
  const teamList = useMemo(() => ["Ultra Hustle Studio", "Design Squad", "Dev Crew"], []);
  ...
  /* ================== ADD-ONS STATE ================== */
  const [addOn, setAddOn] = useState({ name: "", price: "", days: "" });
  ...
  const removeAddOn = (idx) => setAddOns((p) => p.filter((_, i) => i !== idx));

  /* ================== MEDIA (✅ UploadGrid + Success IMPLEMENTED) ================== */
  const [coverFile, setCoverFile] = useState(null); // File object
  const [coverPreview, setCoverPreview] = useState(""); // preview url (string)
  ...
  const onUploadSelect = (selectedFiles) => {
    // selectedFiles = array of File
  ...
  const updateFaq = (idx, key, value) =>
    setFaqs((p) => p.map((item, i) => (i === idx ? { ...item, [key]: value } : item)));
    ...
    <>
              ...
              <div className="create-service-container">
                <div className="csl-stack">
                  ...
                  <div className="csl-card">
                    {/* ... aapka existing Basic Details same ... */}
                    {/* (I left your content unchanged below for brevity) */}
                  ...
                  <div className="am-card">
                    ...
                    {/* ✅ UPDATED: same UploadGrid flow */}
                    <div className="am-uploadBox">
                      {coverPreview ? (
                        <>
                          <img src={coverPreview} alt="cover" className="am-preview" />
                          <button className="am-removeImg" onClick={clearCover} type="button">
                            ×
                          </button>
                          <button
                            type="button"
      ...
      {(uploadStep === "grid" || uploadStep === "success") && (
        <UploadGrid
          ...
          onSelect={(files) => onUploadSelect(files)}
        />
      )}
...
/* ================= UPLOAD GRID (same as your MyPortfolio) ================= */
      ...
      >
        ...
        <div className="flex justify-end items-center mt-3 shrink-0">
          <div className="flex gap-3">
            <button type="button" onClick={onBack} className="upload-btn-cancel px-4 py-2 rounded-lg text-sm border border-black">
              Cancel
            </button>
